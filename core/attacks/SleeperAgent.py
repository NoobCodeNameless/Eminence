...
def batched_step(model, inputs, labels, poison_delta, poison_slices, criterion, target_grad, target_gnorm, augment):
    ...
    poisoned_inputs = inputs + delta_slice
...
class SleeperAgent(Base):
        ...
        for epoch in range(1,epochs+1,1):
            ...
            with torch.no_grad():
                ...
                if testset is not None:
                    predict_digits, labels, _ = self._test(testset, device, batch_size, num_workers, model)
                ...
                if poison_sourceset is not None:
                    predict_digits, labels, _ = self._test(poison_sourceset, device, batch_size, num_workers, model)
                ...
                if poison_testset is not None:
                    predict_digits, labels, _ = self._test(poison_testset, device, batch_size, num_workers, model)
    ...
    def craft_poison_dataset(self, model, log, init_model, trainset, testset, craft_iters, retrain_iter_interval, y_target, y_source, poison_num, augment, batch_size, num_workers, retrain_schedule, eps, device, test_schedule):
        ...
        for t in range(1, craft_iters+1):
            ...
            if t % retrain_iter_interval == 0 and t!= craft_iters: 
                ...
                predict_digits, labels, _ = self._test(source_testset, device, batch_size, num_workers, model)
                source_test_acc = (predict_digits.max(1)[1]==labels).sum().item() * 100. / len(labels)
                
                # test on poisoned testset(source class only)
                predict_digits, labels, _ = self._test(patch_source_testset, device, batch_size, num_workers, model)
                source_test_asr = (predict_digits.max(1)[1]==labels).sum().item() * 100. / len(labels)
                
                # test on benign testset(all classes)
                predict_digits, labels, _ = self._test(self.test_dataset, device, batch_size, num_workers, model)
                test_acc = (predict_digits.max(1)[1]==labels).sum().item() * 100. / len(labels)

                # test on poisoned testset(all classes)
                predict_digits, labels, _ = self._test(full_patch_testset, device, batch_size, num_workers, model)
    ...
    def train(self, schedule=None, init_model=reset_model):
        ...
        if self.current_schedule['benign_training'] is True:
            ...
            if ('pretrain' not in self.current_schedule) or ('pretrain' in self.current_schedule and not os.path.exists(self.current_schedule['pretrain'])):
                self._train_model(model=self.model, 
                                ...
                                augment=augment, 
                                device=device, 
                                schedule=self.current_schedule.get(
                                    'pretrain_schedule', 
                                    {'epochs':100, 'lr':0.1, 'weight_decay': 5e-4,  'gamma':0.1, 'milestones':[50,75], 'batch_size':128, 'num_workers':8, 'momentum': 0.9,}
                                    )
                                )
            ...
            self.poisoned_train_dataset, self.poisoned_source_dataset, self.poisoned_test_dataset = \
                ...
                trainset=self.train_dataset, 
                testset=self.test_dataset, 
                craft_iters=self.current_schedule.get('craft_iters', 250), 
                retrain_iter_interval=self.current_schedule.get('retrain_iter_interval', 50),
                y_target=self.y_target,
                y_source=self.y_source,
                poison_num=int(self.poisoned_rate*len(self.train_dataset)),
                batch_size=self.current_schedule['batch_size'],
                num_workers=self.current_schedule['num_workers'],
                retrain_schedule=self.current_schedule.get(
                    'retrain_schedule',
                    {'epochs':40, 'lr':0.1, 'weight_decay': 5e-4,  'gamma':0.1, 'milestones':[14,24,35], 'batch_size':128, 'num_workers':8, 'momentum': 0.9,}
                    ),
                eps=self.eps,
                device=device,
                augment=augment,
            ...
            epoch_list, acc_list, asr_list = self._train_model(model=self.model, 
                log=log, 
