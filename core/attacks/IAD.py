...
# for all-to-all attack
# class ModifyTarget:
#     def __init__(self, y_target):
#         self.num_classes = 100

#     def __call__(self, targets):
#         return (targets + 1) % self.num_classes
...
class Normalize:
    ...
    def __init__(self, dataset_name, expected_values, variance):
        if dataset_name in ['cifar10', 'cifar100', 'gtsrb', 'tinyimagenet']:
            self.n_channels = 3
...
class IAD(Base):
    ...
    def train(self, schedule=None):
        ...
        scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, self.current_schedule.get('milestones', [33, 66, 100, 133]), self.current_schedule.get('lambda', 0.1))

        # Prepare the backdoor trigger pattern generator
        self.modelG = Generator(self.dataset_name).to(device)
        optimizerG = torch.optim.Adam(self.modelG.parameters(), lr=self.current_schedule.get('lr_G', 0.01), betas=self.current_schedule.get('betas_G', (0.5, 0.9)))
        schedulerG = torch.optim.lr_scheduler.MultiStepLR(optimizerG, self.current_schedule.get('milestones_G', [66, 100, 133, 166]), self.current_schedule.get('lambda_G', 0.1))

        # Prepare the backdoor trigger mask generator
        self.modelM = Generator(self.dataset_name, out_channels=1).to(device)
        optimizerM = torch.optim.Adam(self.modelM.parameters(), lr=self.current_schedule.get('lr_M', 0.01), betas=self.current_schedule.get('betas_M', (0.5, 0.9)))
        schedulerM = torch.optim.lr_scheduler.MultiStepLR(optimizerM, self.current_schedule.get('milestones_M', [3, 7]), self.current_schedule.get('lambda_M', 0.1))
        ...
        for i in range(self.current_schedule['epochs']):
            ...
            acc_list.append(acc_clean)
            asr_list.append(acc_bd)
    ...
    def train_step(
        ...
        for batch_idx, (inputs1, targets1), (inputs2, targets2) in zip(range(len(train_dl1)), train_dl1, train_dl2):
            ...
            bs = inputs1.shape[0]
            # cuz we need to test under very low poisoned rate
            num_bd = max(1, int(self.poisoned_rate * bs))
            # num_cross = max(1, int(self.cross_rate * bs))
            num_cross = num_bd
